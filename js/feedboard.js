...
var linklibrary = ['https://genboy.net/info/feedboard/data/opinion.opml',
  'https://genboy.net/info/feedboard/data/mainstream.opml',
  'https://genboy.net/info/feedboard/data/science.opml',
  'https://genboy.net/info/feedboard/data/tech.opml',
  'https://genboy.net/info/feedboard/data/art.opml',
    ...
    if (valid['chk'] == 1) {
      ...
      for (var i = 0; i < 30; i++) {
        ...
        if (typeof items[i] != 'undefined') {
          ...
          item['description'] = 'No excerpt available';
          if (items[i].getElementsByTagName("description")[0] && items[i].getElementsByTagName("description")[0].firstChild)
    ...
    if (exst == 1) {
      ...
      validateFeedUrl(str, function(valid) {
        if (valid['chk'] == 1) {
          ...
          console.log('!Problemo :' + valid['chk'] + ' (' + str + ')');
        }
      });
  ...
  if (isUrl(url)) {
    var getdoc = loadXMLHTTP(url, function(doc) {
      ...
      if (doc) {
        ...
        valid['chk'] = 'Source not available';
        callback(valid);
      }
    ...
    valid['chk'] = 'Should be a valid feed url';
    callback(valid);
  }
